...
with open("./csv/estaciones.csv", "r") as file:
    ...
    for row in reader:
        if row[9]=="location/lat":
            continue
        #print(f"https://us1.locationiq.com/v1/reverse.php?key={API_KEY}&lat={row[9]}&lon={row[10]}&format=json")
        r = requests.get(f"https://us1.locationiq.com/v1/reverse.php?key={API_KEY}&lat={row[9]}&lon={row[10]}&format=json")
        tmp=dict(r.json())
        # data = {
        #     "lon":tmp["lon"],
        #     "lat":tmp["lat"],
        #     "county":tmp["address"]["county"],
        #     "zipcode":tmp["address"]["postcode"]
        # }
        print(tmp)
...
with open('direcciones.json', 'w') as fout:
